...
class CallingAt(CachingHotspot):
    ...
    def destinations(self, destinations):
        ...
        if destinations and len(destinations) > 0:
            ...
            if len(destinations) == 1: # one station, "a"
                text = destinations[0]
    ...
    def should_redraw(self):
        if self.last_updated == 0.0 or self.labelDirty:
            needs_redraw = True
        elif not self.needs_scroll:
            needs_redraw = False
        else:
            needs_redraw = time.perf_counter() - self.last_updated > self.interval
        # print("Needs redraw? {}".format(needs_redraw))
        return needs_redraw
    ...
    def buildCache(self):
        ...
        # print("Created {} \"{}\", needs_scroll={}".format(self.image, self.text, self.needs_scroll))
